...
// Generate Daily Todos Route
router.post('/generate-daily-todos', async (req, res) => {
  if (!process.env.GEMINI_API_KEY) {
    return res.status(500).json({ error: 'GEMINI_API_KEY not configured on server' });
  }
  const result = await generateDailyTodos(req.body);
  res.status(200).json(result);
});

// Generate AI-Powered Smart Timetable Slots
router.post('/generate-timetable-slots', async (req, res) => {
  if (!process.env.GEMINI_API_KEY) {
    return res.status(500).json({ error: 'GEMINI_API_KEY not configured on server' });
  }
...
- Ensure the start times are chronologically sorted.`;
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to generate timetable slots' });
  }
});

// Generate AI Study Suggestions Route
router.post('/ai-suggestions', async (req, res) => {
  ...
  const categoryText = categories
    .map((c) => `  - ${c.name}: ${c.progress}% complete (${c.totalTopics} topics)`)
    .join('\n');

  const slotsText = todaySlots
    .map((s) => `  - ${s.label} (${s.time}, ${s.durationMins} mins, type: ${s.slotType})`)
    .join('\n');
